        ...
        <div>
            <b-form-group name="form-group" id="form-group" label="Choose your DAAC:"><br>
                ...
                <div>
                    ...
                    <div class="radio_div">
                        <b-form-radio v-for="(item, index) in daacs" :key=index :name="item.short_name.replace(' ', '_')" :id="item.short_name.replace(' ', '_') + '_' + index" :value="item.long_name" @click="setSelectedValues(item.url, item.short_name, item.long_name, item.description)" v-model="selected">
                            {{ item.short_name }}
                        </b-form-radio>
                    </div>
                </div>
        ...
        methods: {
            ...
            getDaac(daac_specific){
                ...
                for(var dict in this.daacs) {
                    ...
                    if(daac_specific.toLowerCase() === long_name.toLowerCase() || daac_specific.toLowerCase() === short_name.toLowerCase()){
                        let url = this.daacs[dict]['url']
                        let description = this.daacs[dict]['description']
                        return { 'short_name':short_name, 'long_name': long_name, 'url':url, 'description':description }
                    }
                }
            },
            // @vuese
            // On selected, sets current daac objects from values
            setCurrentDaacObjects(current_daac, url, short_name, long_name, description){
                //console.log('set current daac objects')
                var daac_specific_data
                if(typeof url == 'undefined' || typeof short_name == 'undefined' || typeof long_name == 'undefined' || typeof description == 'undefined'){
            ...
            GetCurrentDaacAndUpdate(){
                ...
                if((typeof this.$route!= 'undefined' && typeof this.$route.params.default != 'undefined' && this.$route.params.default!=null && this.$route.params.default !='' && this.$route.params.default !='selection') || this.selected != '' || window.localStorage.getItem('DAAC')!= null){
                    ...
                    if(this.selected !=''){ 
                        ...
                        selected = this.getDaac(this.$route.params.default.replace(/_/g,' ').toUpperCase())
